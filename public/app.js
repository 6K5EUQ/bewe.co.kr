...
function keplerPos(sat, timeMs) {
    ...
    const xeci = cr * x2 - sr2 * y2;
    const yeci = sr2 * x2 + cr * y2;
    const zeci = z2;

    const th = gmstRad(timeMs);
    const cT = Math.cos(th), sT = Math.sin(th);
    const xe =  cT * xeci + sT * yeci;
    const ye = -sT * xeci + cT * yeci;
    const ze = zeci;
...
// Color by altitude — matches BE_WE sat_view.cpp
function satColor(alt_km) {
    if (alt_km < 2000)  return [1.0, 0.31, 0.20]; // LEO: red
    if (alt_km < 35000) return [1.0, 0.86, 0.20]; // MEO: yellow
    return [0.86, 0.86, 1.0];                       // GEO: white-blue
}
...
let satTleData = [];  // raw TLE elements
...
function buildOrbitPath(sat) {
    ...
    const period_s = 86400 / sat.n;  // seconds
    const steps = 360;
    const dt_s = period_s / steps;
    const now = Date.now();
    const t0 = now - (period_s * 1000) / 2;

    const pts = [];
    for (let i = 0; i <= steps; i++) {
        const t = t0 + i * dt_s * 1000;
        const p = keplerPos(sat, t);
        const v = latLonToVec3(p.lat, p.lon, GLOBE_RADIUS * (1 + p.alt_km / 6378.137));
        pts.push(v);
    }

    const color = satColor(keplerPos(sat, now).alt_km);
...
function showOrbit(idx) {
    ...
    selectedSatIdx = idx;
    const line = buildOrbitPath(satTleData[idx]);
    orbitGroup.add(line);
}

// Satellite mode: 'off' | 'visual' | 'starlink'
let satMode = 'visual';
let satPosUpdateTimer = 0;
...
function updateSatPositions() {
    if (satMode === 'off' || satTleData.length === 0) return;
    const now = Date.now();
    const count = Math.min(satTleData.length, MAX_SATS);
    for (let i = 0; i < count; i++) {
    ...
    // Rebuild orbit if a satellite is selected
    if (selectedSatIdx >= 0) {
        clearOrbit();
        showOrbit(selectedSatIdx);
    }
}

// Initial load
loadSatellites('visual');
// Refresh TLE every hour, recompute positions every 30s
setInterval(() => loadSatellites(satMode !== 'off' ? satMode : null), 3600000);
setInterval(() => updateSatPositions(), 30000);

// ── Satellite name tooltip state ────────────────────────────────────────────
let satTooltipIdx = -1;
...
function projectWorldPoint(lat, lon, r) {
    const pos = latLonToVec3(lat, lon, r);
    pos.applyEuler(new THREE.Euler(rotX, rotY, 0, 'XYZ'));
    const projected = pos.clone().project(camera);
    if (projected.z > 1) return null;
    return {
        sx: (projected.x * 0.5 + 0.5) * window.innerWidth,
        sy: (-projected.y * 0.5 + 0.5) * window.innerHeight,
...
function getHoveredStation(e) {
    ...
    let bestIdx = -1;
    stations.forEach((st, i) => {
        const p = projectWorldPoint(st.lat, st.lon, GLOBE_RADIUS * 1.005);
        if (!p) return;
...
function getHoveredSat(e) {
    if (satMode === 'off' || satTleData.length === 0) return -1;
    const count = Math.min(satTleData.length, MAX_SATS);
    const now = Date.now();
    let bestDist = 20;
    let bestIdx = -1;
    for (let i = 0; i < count; i++) {
        const p = keplerPos(satTleData[i], now);
        const proj = projectWorldPoint(p.lat, p.lon, GLOBE_RADIUS * (1 + p.alt_km / 6378.137));
        if (!proj) continue;
...
function handleHover(e) {
    ...
    if (satIdx >= 0) {
        ...
        const altStr = p.alt_km < 1000
            ? Math.round(p.alt_km) + ' km'
            : (p.alt_km / 1000).toFixed(1) + ' Mm';
...
function handleClick(e) {
    ...
    if (stIdx >= 0) {
        clearOrbit();
        showPopup(stations[stIdx]);
        return;
    }

    const satIdx = getHoveredSat(e);
    if (satIdx >= 0) {
        if (selectedSatIdx === satIdx) {
            clearOrbit();
        } else {
            showOrbit(satIdx);
        }
...
function showPopup(st) {
    ...
    const hostName = st.station_id.includes('_')
        ? st.station_id.split('_').slice(1).join('_')
        : st.station_id;
    ...
    // v2 fields
    const freqEl  = document.getElementById('popup-freq');
    const srEl    = document.getElementById('popup-sr');
    const chEl    = document.getElementById('popup-ch');
    const recEl   = document.getElementById('popup-rec');
    const v2Wrap  = document.getElementById('popup-v2');

    const freq = fmtFreqHz(st.center_freq_hz);
    const sr   = fmtSrHz(st.sample_rate_hz);
    ...
    if (hasV2 && v2Wrap) {
        ...
        if (chEl)   chEl.textContent   = (st.channel_count || 0) + ' ch';
        if (recEl) {
            recEl.textContent = st.hist_recording ? 'REC' : '';
            recEl.style.display = st.hist_recording ? '' : 'none';
        }
    } else if (v2Wrap) {
